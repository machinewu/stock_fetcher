...
def fetch(url, headers=None):
    ...
    conn = common_connection(re.search(r'^(?:https?://)?([^/\?#=%]+)', url, re.I).group(1))
...
def external_futures(page_stock_code):
    ...
    if page_stock_code not in check_code:
    ...
    if status_code < 400 and data is not None:
        return Stock.external_futures_make(re.sub(r'^var [^=]+="', '', str(data)).rstrip('";\n').split(','), detail_url)
...
def inner_future(page_stock_code):
    ...
    (status_code, data) = fetch(url, {'Referer': detail_url})
    if status_code < 400 and data is not None:
        return Stock.internal_futures_make(re.sub(r'^var [^=]+="', '', str(data)).rstrip('";\n').split(','), detail_url)
...
def cn_stock(page_stock_code):
    ...
    (status_code, data) = fetch(url, {'Referer': detail_url})
    if status_code < 400 and data is not None:
        return Stock.cn_stock_make(re.sub(r'^var [^=]+="', '', str(data)).rstrip('";\n').split(','), detail_url)
...
def hk_stock(page_stock_code):
    ...
    (status_code, data) = fetch(url, {'Referer': detail_url})
    if status_code < 400 and data is not None:
        return Stock.hk_stock_make(re.sub(r'^var [^=]+="', '', str(data)).rstrip('";\n').split(','), detail_url)
...
def us_stock(page_stock_code):
    ...
    if status_code < 400 and data is not None:
        print data
        return Stock.us_stock_make(re.sub(r'^var [^=]+="', '', str(data)).rstrip('";\n').split(','), detail_url)
...
def exchange_rate_stock(page_stock_code):
    ...
    (status_code, data) = fetch(url, {'Referer': detail_url})
    if status_code < 400 and data is not None:
        return Stock.exchange_rate_make(re.sub(r'^var [^=]+="', '', str(data)).rstrip('";\n').split(','), detail_url)


def test():
    external_futures('CL').display_print()
    # inner_future('AU0').display_print()
    # cn_stock('sz300582').display_print()
    # hk_stock('01810').display_print()
    # us_stock('AMD').display_print()
    # exchange_rate_stock('USDCNY').display_print()
